...
export default function MarkdownMessage({ content }: MarkdownMessageProps) {
  if (!content) {
    return <div className="text-zinc-500 dark:text-zinc-400 italic">No content</div>;
  }
  ...
  const components: Components = {
    ...
    code: ({ inline, className, children, ...props }) => {
      return !inline ? (
        <code
          className={`block bg-zinc-800 dark:bg-zinc-900 text-zinc-100 p-3 rounded-lg overflow-x-auto mb-3 text-sm ${className || ''}`}
          {...props}
        >
          {children}
        </code>
    ...
    th: ({ ...props }) => (
      <th className="border border-zinc-300 dark:border-zinc-600 px-3 py-2 text-left font-semibold" {...props} />
    ),
    td: ({ ...props }) => (
      <td className="border border-zinc-300 dark:border-zinc-600 px-3 py-2" {...props} />
    ),
    hr: ({ ...props }) => (
