...
export default function ChatPage() {
  ...
  const saveConversation = async (messageList: Message[]) => {
    ...
    try {
      const title = messageList[0]?.content.substring(0, 50) || 'Ollama Chat';
      ...
      <div className="bg-white dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 px-6 py-4">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            {loadingModels ? (
              ...
              <div className="text-sm text-red-600 dark:text-red-400">
                No models found - install Ollama
              </div>
            ) : (
              <select
                value={model}
                onChange={(e) => setModel(e.target.value)}
                className="px-3 py-1 text-sm rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white"
              >
          ...
          <div className="flex gap-2 items-center">
            ...
            <Link
              href="/conversations"
              className="px-4 py-2 text-sm bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-lg hover:bg-zinc-700 dark:hover:bg-zinc-200 transition-colors"
            >
              View All
            </Link>
          </div>
